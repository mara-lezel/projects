...
class Plotting:
    ...
    def plot(self, setting, r, grid, counter, play_count = 1, show = False):
        ...
        #self.y = self.get_ema(self.y)
        self.won = self.get_ema(self.y)

        file1 = "/Users/mara/Desktop/KP kodas/basic q-learning/data/atlygis" + str(grid) + ".txt"
        file2 = "/Users/mara/Desktop/KP kodas/basic q-learning/data/noise" + str(grid) + ".txt"
        ...
        match setting:
            case "CML":
                ylabel = "Total reward"
                title = f"Total Reward per Episode   [R={r}] [Grid {grid}]"
            case "AVG":
                ylabel = "Average reward"
                title = f"Average Reward after {play_count} test runs   [R={r}] [Grid {grid}]"
        ...
        if show:
            ax4.legend(handles=p1+p2+p3+p4, loc='center', bbox_to_anchor=[0.9, 0.2])
